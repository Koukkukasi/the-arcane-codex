"""
The Arcane Codex - Database Layer with Connection Pooling
Production-optimized SQLite with connection management
"""
...
class ConnectionPool:
    ...
    def get_connection(self):
        ...
        try:
            ...
            try:
                ...
                if wait_time > 1.0:
                    logger.warning(f"Long connection wait: {wait_time:.2f}s")
    ...
    def get_stats(self) -> Dict[str, Any]:
        """Get pool statistics"""
        avg_wait = sum(self.connection_wait_times) / len(self.connection_wait_times) if self.connection_wait_times else 0
        ...
        return {
            ...
            'max_wait_time_ms': max(self.connection_wait_times) * 1000 if self.connection_wait_times else 0
        }
...
class ArcaneDatabase:
    """
    Complete database system with connection pooling
    """

    def __init__(self, db_path="arcane_codex.db", pool_size=20, timeout=30.0):
        self.db_path = db_path
        self.pool = ConnectionPool(db_path, pool_size, timeout)
        self.init_database()

    def init_database(self):
        """Create all required tables with optimized indexes"""
        with self.get_connection() as conn:
            conn.executescript("""
                ...
                CREATE TABLE IF NOT EXISTS players (
                    ...
                    ai_controlled INTEGER DEFAULT 0,
                ...
                CREATE INDEX IF NOT EXISTS idx_whispers_game_turn ON sensory_whispers(game_id, turn);
                CREATE INDEX IF NOT EXISTS idx_whispers_player ON sensory_whispers(player_id);

                CREATE INDEX IF NOT EXISTS idx_history_game_turn ON game_history(game_id, turn);
                CREATE INDEX IF NOT EXISTS idx_history_type ON game_history(event_type);
    ...
    def all_players_acted(self, game_id: str) -> bool:
        ...
        with self.get_connection() as conn:
            player_count = conn.execute("""
                ...
                WHERE game_id = ? AND status = 'active' AND ai_controlled = 0
